...
| `trusted_roots_path` | `string` | TrustedRootsPath optionally points at a PEM bundle of the Registrar's root certificates. When set, the WRPRC's own x5c chain is validated against it at startup and its entitlements are checked for consistency with the access certificate. When unset, only the document's structure is checked, since chain validation without trust anchors would be theatre. | -                                        | -       | No       |
