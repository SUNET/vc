...
| `clock_skew_seconds`   | `int`      | How far the token's exp may lie in the past and still be accepted, for a client whose clock runs behind ours. The underlying verifier (coreos/go-oidc) has no leeway setting, so this is applied by moving the clock it reads. That clock serves two checks, and the effect on each is opposite: exp is relaxed by this much, and go-oidc's own fixed five-minute nbf leeway is reduced by the same amount. Keep it well under five minutes, or a token whose nbf is legitimately a little in the future stops being accepted. iat is not validated at all. Zero disables both effects, and the upper bound is one second short of go-oidc's nbf leeway - at or past it the leeway is gone and a token with a legitimately future nbf starts being rejected, which is the opposite of what raising a skew tolerance is meant to achieve. | -       | `60`                | No       |
