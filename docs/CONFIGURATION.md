...
| `credential_types`  | `[]string` | The W3C Verifiable Credential types this scope issues, most general first - conventionally starting with the base "VerifiableCredential" and narrowing from there, e.g. ["VerifiableCredential", "DiplomaCredential"]. Used only for the W3C formats (ldp_vc, vc+ld+json, jwt_vc_json). It is the single source for a type list that has to agree in three places: the issuer metadata's credential_definition.type, the types issueVC20 mints into the credential (which it reads back from that metadata), and the DCQL meta.type_values a verifier constrains a request by. Without it there is no way to ask for such a credential: DCQL requires type_values for these formats, and the only value available was the hardcoded base type, which as a constraint matches EVERY W3C credential in a wallet rather than the intended one (SUNET/vc#680). Leaving it unset keeps exactly that behaviour for issuance, and a verifier reports the scope as having no expressible constraint rather than emitting an over-broad query - see DCQLMetaQuery. | -             | -           | No                                                                               |
