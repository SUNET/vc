...
// TestContextLoaderRefusesRedirects pins the second hop.
//
// The issuer allowlists which context URLs it may dereference. If the fetch
// follows redirects, an allowlisted endpoint can answer 302 and send it
// somewhere that was never allowed - which is how an allowlist checked only on
// the first hop gets bypassed. A redirecting context must fail to load.
func TestContextLoaderRefusesRedirects(t *testing.T) {
	...
	require.Error(t, err, "a redirect while loading a context must not be followed")
...
// loads, so the policy blocks the bypass and not the feature.
