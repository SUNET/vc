...
func NewValidator() (*validator.Validate, error) {
	...
	//
	// doc:constraint name="mongo_uri_required" struct="Common" applies="Mongo,SQL,HA" description="Mongo.URI is required for services that open a MongoDB store (apigw, registry, verifier) when SQL.Backend is 'mongo' (the default primary-store backend) or when HA.Enable is true (HA caching has no relational backend yet, so it always uses Mongo). Not required for a pure relational deployment (a non-mongo SQL.Backend with HA disabled), nor for the issuer, which uses no database. Enforced in configuration.New rather than as a struct validation, since it depends on the running service."
