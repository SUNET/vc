...
// TestMSOBuilder_AddDataElement_PseudonymSeedUsesShorterSalt guards the
// pseudonym_seed-specific 8-byte salt branch in AddDataElement, which had no
// test coverage: an 8-byte-too-many salt would silently push the encoded
// IssuerSignedItem past the Longfellow ZK circuit's 128-byte item limit.
func TestMSOBuilder_AddDataElement_PseudonymSeedUsesShorterSalt(t *testing.T) {
	builder := NewMSOBuilder(DocType)
